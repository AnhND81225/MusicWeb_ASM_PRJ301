...
public class SubscriptionDAO {
    ...
    public List<SubscriptionDTO> getAllSubscriptions() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            list = session.createQuery("FROM tblSubscription", SubscriptionDTO.class).list();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return list;
    ...
    // Tìm gói theo tên (vd: “Premium”)
    public SubscriptionDTO getByName(String name) {
        SubscriptionDTO sub = null;
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<SubscriptionDTO> query = session.createQuery(
                    "FROM tblSubscription WHERE nameSubscription = :name", SubscriptionDTO.class);
